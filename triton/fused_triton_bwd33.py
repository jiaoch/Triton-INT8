...
# FilePath: \undefinedc:\Users\hp\Desktop\fused.py
# Release version: 1.0
# Date: 2025-09-01
# Author: FanJH
...
def fused_int8_gemm_kernel(
    ...
    for k in range(0, K, BLOCK_K):
        ...
        b_bf_ptrs = B_bf_ptr + pid_b * stride_bb + offs_n[None, :] * stride_bn + (k + offs_k[:, None]) * stride_bk
        ...
        amax_b = tl.max(tl.abs(b_bf)) + 1e-5
        scale_b = 127.0 / amax_b
        b_q = tl.floor(b_bf * scale_b + 0.5).to(tl.int8)
    
        acc += tl.dot(a_q, b_q) / (scale_a * scale_b)
...
def fused_int8_gemm(A_bf, B_bf):
    ...
    fused_int8_gemm_kernel[grid](
        ...
        stride_bb=B_bf.stride(0), stride_bn=B_bf.stride(1), stride_bk=B_bf.stride(2),
